...
public interface AwsMqttConstants {
	...
	final static String DEVICE_DEFENDER_JSON_TOPIC_WILDCARD = "$aws/things/<gatewayHid>/defender/metrics/json/+";
	final static String DEVICE_DEFENDER_JSON_TOPIC_REGEX = "\\$aws/things/\\w+/defender/metrics/json/\\w+";

	static String deviceDefenderJsonTopic(String gatewayHid) {
		return DEVICE_DEFENDER_JSON_TOPIC.replace("<gatewayHid>", gatewayHid);
	}

	static String deviceDefenderJsonTopicWildcard(String gatewayHid) {
		return DEVICE_DEFENDER_JSON_TOPIC_WILDCARD.replace("<gatewayHid>", gatewayHid);
	}
